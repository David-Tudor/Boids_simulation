...
def key_value(x,y, len_grid_x, len_grid_y):
    """
...
@cython.boundscheck(False)
def main( int num_boids, int num_hawks, int grid_size ):
    """
    ...
    They are also written to output_mpi.npz along with t_record, data_record, hawk_record, cumulative_num_eaten.
    ...
    cdef:
        ...
        int[:] nearby_boid_is = np.zeros(num_boids, dtype=np.int32)
        int[:] nearby_hawk_is = np.zeros(num_hawks, dtype=np.int32)
    ...
    cdef:
        ...
        int [::1] rank_boid_is = np.zeros(num_boids, dtype=np.int32)
    ...
    while t < t_max:
        ...
        if rank == MANAGER:
            data_holder = np.zeros((num_boids, 4), dtype=np.double)
        else:
            data_holder = None
            
            
        # MPI Broadcast
        comm.Bcast([data, MPI.DOUBLE], root=MANAGER)
        new_data = data 

        if num_hawks != 0:
            comm.Bcast([hawk_data, MPI.DOUBLE], root=MANAGER)
            new_hawk_data = hawk_data
            comm.Bcast([is_boid_j_eaten, MPI.INT], root=MANAGER)
        ...
        # Make a list which cell each boid is in and reorder so the cell keys are ascending.
        # From this list, record the index each cell starts at and its length.
        flat_grid =[]
        for j in range(num_boids):
            boid = data[j]
            flat_grid.append( (int(key_value(boid[0]//grid_size, boid[1]//grid_size, len_grid_x, len_grid_y)), j) )
        ...
        if num_hawks != 0:
            ...
            for j in range(num_hawks):
                hawk = hawk_data[j]
                hawk_grid.append( (int(key_value(hawk[0]//grid_size, hawk[1]//grid_size, len_grid_x, len_grid_y)), j) )
        ...
        rank_work_allocation = np.zeros(num_grid_cells, dtype=np.int64)
        rank_current_work = np.zeros(threads, dtype=np.int64)
        
        # Allocate each grid cell to the rank with the least current work.
        for i in range(num_grid_cells):
            chosenThread = np.argmin(rank_current_work)
            rank_work_allocation[i] = chosenThread
            rank_current_work[chosenThread] += lens_list[i]


        # Track the order boids are calculated in as later Gatherv will change the order.
        rank_boid_is = np.zeros(num_boids, dtype=np.int32)
        rank_boid_num = 0

        # Loop over every cell in the grid.
        for cell_key in range(num_grid_cells):

            # Only do its rank's work.
            if rank_work_allocation[cell_key] != rank:
                continue
            ...
            for i in range(-1,2):
                for j in range(-1,2):
                    ...
                    for grid_i in range(cell_len):
                        ...
                        nearby_boid_is[num_grid_boids] = indices_list[resultant_i]
                        num_grid_boids += 1
                    ...
                    if num_hawks != 0:
                        ...
                        for grid_i in range(cell_len):
                            resultant_i = cumulative_lens_i + grid_i
                            nearby_hawk_is[num_grid_hawks] = hawk_indices_list[resultant_i]
                            num_grid_hawks += 1
            ...
            for i in range(centre_cell_len):
                ...
                boid_i = nearby_boid_is[resultant_i]

                rank_boid_is[rank_boid_num] = boid_i
                rank_boid_num += 1
                ...
                # Check if any hawks are in vision.
                if num_hawks != 0:
                    for j in range(num_grid_hawks):
                        hawk_i = nearby_hawk_is[j]
                ...
                if new_data[boid_i,2] == 0.0 and new_data[boid_i,3] == 0:
                    new_data[boid_i,2], new_data[boid_i,3] = data[boid_i,2], data[boid_i,3]

                # Make speeds the same.
                speed = sqrt(new_data[boid_i,2]*new_data[boid_i,2] + new_data[boid_i,3]*new_data[boid_i,3])
                if speed == 0:
                    speed = initial_spd
                new_data[boid_i,2], new_data[boid_i,3] = new_data[boid_i,2]*initial_spd/speed, new_data[boid_i,3]*initial_spd/speed

                new_data[boid_i,0] += new_data[boid_i,2] * dt
                new_data[boid_i,1] += new_data[boid_i,3] * dt

                # Periodic boundaries.
                new_data[boid_i] = apply_periodic_boundaries(new_data[boid_i], max_x, max_y)



        rank_boid_is = np.int32(rank_boid_is[:rank_boid_num])
        new_data_portion = np.array(new_data, dtype=np.double)[rank_boid_is]

         # Flatten for Gatherv.
        new_data_portion_flat = new_data_portion.ravel()

        if rank == MANAGER:
            gather_lengths = np.int64(rank_current_work) * 4 # *4 as ravelled.
            gather_displacements = np.append(0, np.cumsum(gather_lengths[:-1]))
            data_holder = np.zeros(sum(gather_lengths), dtype=np.double)

            gather_lengths2 = np.int64(rank_current_work)
            gather_displacements2 = np.append(0, np.cumsum(gather_lengths2[:-1]))
            boid_is_order = np.zeros(num_boids, dtype=np.int32)

        else:
            gather_lengths = None
            gather_displacements = None
            data_holder = None
            gather_lengths2 = None
            gather_displacements2 = None
            boid_is_order = None

        # Gather the new boid positions and the order each rank calculated its allocated boids in.
        comm.Gatherv([new_data_portion_flat, MPI.DOUBLE], [data_holder, gather_lengths, gather_displacements, MPI.DOUBLE], root=MANAGER)
        comm.Gatherv([rank_boid_is, MPI.INT], [boid_is_order, gather_lengths2, gather_displacements2, MPI.INT], root=MANAGER)


        if rank == MANAGER:
            data_holder = data_holder.reshape(num_boids, 4)

            # data_holder in MANAGER currently ordered as the new_data_portions from each rank next to each other.
            # The indices need to be reordered to be the same as before.
            data = data_holder[np.argsort(boid_is_order), :]
            ...
            for i in range(num_hawks):
                ...
                hawk_data[i,0] += new_hawk_data[i,2] * dt
                hawk_data[i,1] += new_hawk_data[i,3] * dt
